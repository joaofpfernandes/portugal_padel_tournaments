...
function getPlayerInfo(name, club, rankings) {
  ...
  if (!matches.length) return { points: 0, level: 6 };
  if (matches.length === 1) return { points: parsePoints(matches[0].Points), level: parseLevel(matches[0].Level) };
  const clubMatch = matches.find((r) => r.Club?.toLowerCase().trim() === club.toLowerCase().trim());
  if (clubMatch) return { points: parsePoints(clubMatch.Points), level: parseLevel(clubMatch.Level) };
  return { points: 0, level: 6 };
}
